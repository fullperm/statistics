/*
 ...
 Description:
 This function fetches data from the Secondlife API to retrieve the current number of signups
...
function SignupsSL() {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (lines[i].trim() === "signups_updated_slt" && i + 1 < lines.length) {
            time = lines[i + 1].trim();
        } else if (lines[i].trim() === "signups" && i + 1 < lines.length) {
            signups = lines[i + 1].trim();
        }
    }
    ...
    sheet.appendRow([signups, customTime]);
}
...
function createTrigger() {
    ...
    ScriptApp.newTrigger('SignupsSL')
        .timeBased()
        .everyMinutes(TIMER)
        .create();
}
...
function onOpen() {
    ...
    ui.createMenu('Actions')
        .addItem('Start SignupsSL', 'SignupsSL')
        .addItem('Start SignupsSL every ' + TIMER + ' minutes', 'createTrigger')
        .addItem('Stop SignupsSL Timer', 'stopTrigger')
        .addToUi();
}
